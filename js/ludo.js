...
const LudoGame = (() => {
  ...
  function _startGameState(players, gameMode, playerTokens) {
    ...
    state = {
      ...
      turnIdx:      0,
      die:          null,
      dieRaw:       [],   // individual die values: [d1] for Kids, [d1, d2] for Classic
      rolled:       false,
      moving:       false,
      over:         false,
      startTime:    Date.now(),
    ...
    // Show/hide second die based on mode
    const die2El = document.getElementById('ludo-dice-val-2');
    if (die2El) die2El.style.display = (gameMode === 'classic') ? '' : 'none';
  ...
  async function rollDice() {
    ...
    // Build a human-readable roll label
    const raw = state.dieRaw || [die];
    const rollLabel = raw.length === 2 ? `${raw[0]}+${raw[1]}=${die}` : `${die}`;
    // Bonus turn: Kids = single 6; Classic = no bonus turn
    const bonusTurn = state.gameMode === 'kids' ? raw[0] === 6 : false;

    if (total === 0) {
      setStatus(`${_pEmoji(cp)} ${cp.name} rolled ${rollLabel} — no moves. Next player!`, cp.colors[0]);
      await pause(800);
      endTurn(!bonusTurn);
    } else if (total === 1) {
      ...
      setStatus(`${_pEmoji(cp)} ${cp.name} rolled ${rollLabel}! Tap a piece to move.`, cp.colors[0]);
      renderPieces();
    }
  }
  ...
  async function movePiece(color, pi, die) {
    ...
    const raw = state.dieRaw || [die];
    const canEnter = state.gameMode === 'kids'
      ? (raw[0] === 1 || raw[0] === 6)
      : raw.some(d => d === 6);
    ...
    state.moving = false;
    // Bonus turn: Kids Mode only — single die was 6; Classic always switches
    const bonusTurnMove = state.gameMode === 'kids' ? (state.dieRaw?.[0] === 6) : false;
    endTurn(!bonusTurnMove);
  }
  ...
  async function computerTurn() {
    ...
    const raw = state.dieRaw || [die];
    const rollLabel = raw.length === 2 ? `${raw[0]}+${raw[1]}=${die}` : `${die}`;
    setStatus(`🤖 Computer rolled ${rollLabel}`, 'gray');
    await pause(400);
    ...
    if (!best) {
      setStatus(`🤖 Computer rolled ${rollLabel} — no moves!`, 'gray');
      await pause(700);
      const bonusTurnCpu = state.gameMode === 'kids' ? raw[0] === 6 : false;
      endTurn(!bonusTurnCpu);
      return;
    }
  ...
  function getMoveable(color, die) {
    return state.pieces[color].reduce((acc, pos, pi) => {
      ...
      // Entry rule: Kids = roll 1 or 6 on single die; Classic = either of 2 dice shows a 6
      const raw = state.dieRaw || [die];
      const canEnter = state.gameMode === 'kids'
        ? (raw[0] === 1 || raw[0] === 6)
        : raw.some(d => d === 6);
  ...
  function animateDice() {
    return new Promise(resolve => {
      ...
      // Roll independent values for each die
      const result1 = Math.floor(Math.random() * 6) + 1;
      const result2 = isClassic ? Math.floor(Math.random() * 6) + 1 : 0;

      // Store raw die values for entry / bonus-turn checks
      state.dieRaw = isClassic ? [result1, result2] : [result1];
      ...
      if (el1) el1.classList.add('dice-rolling');
      if (el2 && isClassic) el2.classList.add('dice-rolling');
      window.SFX?.play('click');
      ...
      function tick() {
        ...
        if (progress >= 1) {
          if (el1) { el1.classList.remove('dice-rolling'); el1.textContent = faces[result1 - 1]; }
          if (el2) {
            el2.classList.remove('dice-rolling');
            el2.textContent = isClassic ? faces[result2 - 1] : '🎲';
          }
          resolve(isClassic ? result1 + result2 : result1);
          return;
        }

        if (el1) el1.textContent = faces[Math.floor(Math.random() * 6)];
        if (el2 && isClassic) el2.textContent = faces[Math.floor(Math.random() * 6)];
